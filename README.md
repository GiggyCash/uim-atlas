...
Pre-alpha. Live Planner v1 loads the production Recipe for Disaster goal and all 41 indexed production methods, evaluates the current UIM snapshot, and shows one actionable method or ready manual quest handoff in a compact sidebar. Its v1.1 correction prioritizes ready goal-frontier milestones, then methods that directly clear a blocked frontier prerequisite, before general long-term goal training. Unknown, stale, blocked and unresolved-preparation candidates cannot occupy the primary recommendation. Methods display `SETUP READY`, a structured start location and an explicit unverified-travel note. Text wraps to the actual sidebar width, alternatives use separate title/status/reason lines, and raw catalog editorial prose is excluded from normal explanations. The panel clears on logout, loading, world hop, profile/account change and plugin shutdown. See [the Live Planner contract and smoke test](docs/live-planner-v1.md).

A domain-only recommendation foundation loads three synthetic test methods, evaluates hard requirements and missing preparation with explicit freshness, and scores eligible methods using visible placeholder weights. See [the implemented schema](DATA_SCHEMA.md#implemented-foundation-synthetic-method-schema-v1). Fixtures exist only in test resources and cannot load as production advice. Live planning uses only production catalogs.

An immutable `AccountStateFacts` adapter exposes observed real skill levels/XP, quest completion/started state, Quest points, inventory occupancy/free slots, exact item quantities in inventory, equipment and combined carried scopes, and verified capabilities through a generic `FactLookup`. It preserves observation provenance and freshness, including unknown state and conservative combined quantities. See [the fact-ID contract](DATA_SCHEMA.md#account-state-fact-contract). `PlanningService` creates it from each live snapshot.

The domain-only `MethodRanker` evaluates multiple candidates, reuses `MethodScorer`, and selects an optional best plus ordered eligible alternatives. `AVAILABLE` and `NEEDS_PREP` compete by descending score, then ascending method ID; `BLOCKED` and `UNKNOWN` retain unscored diagnostics and cannot win. Results preserve missing preparation, original scoring inputs and contribution breakdowns. Synthetic tests cover deterministic ordering, UIM scoring tradeoffs and an inventory change reversing the winner through the full account-state/fact/evaluation/scoring chain. Live planning consumes this result through the existing goal and strategic layers.

`SetupScoringInputs` derives current setup fit, setup cost, transition cost and inventory disruption from method metadata and the observed quantities/free capacity exposed by `FactLookup`. Missing quantities, preparation burden and slot pressure use a small [documented formula](DATA_SCHEMA.md#account-specific-setup-scoring). Unusable observations produce an explicit unresolved result with provenance, never favorable default scores. The other five factors remain explicitly supplied. The full-chain synthetic test uses this derivation: current setup beats a slightly faster alternative, and an inventory change reverses the winner. Live production planning delegates to the same component.

The first [production Construction catalog](src/main/resources/uimatlas/methods/construction-v1.json) bundles three sourced methods: novice oak Mahogany Homes, adept teak contracts, and limestone attack stones with an existing flamtaer bag. [Production schema v3](DATA_SCHEMA.md#production-construction-catalog-v1-schema-v3) adds immutable source/review metadata, optional setup, working-capacity diagnostics, conditional efficiency profiles, explicit slot semantics, and a canonical item-ID validation boundary. Current Wiki pages and upstream item IDs were reviewed on 2026-09-14. XP ranges are conditional on a verified matching profile, setup quantities are starting checks, and storage value is neutral. A positive RuneLite server house-location value verifies `capability.poh.owned`; zero or unavailable state stays UNKNOWN because RuneLite does not document a verified non-ownership value. Further limestone access/preparation checks remain unsupported. Tests verify these boundaries and the built JAR's production/test separation. The live planner may select these methods only when their generic facts and actionability are verified.

Construction Readiness v1 composes the existing production catalog through evaluation, setup inputs, efficiency, scoring/ranking, preparation feasibility and a separate actionability gate. A higher-scoring method with unresolved preparation remains diagnostic and cannot displace a lower-scoring ready method. A carried plank sack can supply verified typed contents and free capacity through RuneLite's named server varbits; absence or a failed/invalid read stays UNKNOWN. Capacity remains separated into hard/preparation constraints, diagnostic working batches and optional verified profiles. Live Planner v1 preserves that same gate.
