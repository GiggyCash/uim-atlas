...
An immutable `AccountStateFacts` adapter exposes observed real skill levels/XP, inventory occupancy/free slots, exact item quantities in inventory, equipment and combined carried scopes, and verified POH ownership through a generic `FactLookup`. It preserves observation provenance and freshness, including unknown state and conservative combined quantities. See [the fact-ID contract](DATA_SCHEMA.md#account-state-fact-contract). The adapter is not yet used for live recommendation selection.
