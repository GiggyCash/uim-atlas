...
Eligible ordering is descending total score, then ascending method ID using Java `String` natural order. There is no status bonus, approximate tie threshold or minimum winning score. Excluded diagnostics are also ordered by ID, and duplicate candidate IDs are errors. This makes both lists independent of input order without a switching policy. The ranker reuses the existing scorer's eligibility and weights. Callers must provide a coherent immutable fact snapshot and scoring inputs derived for that snapshot; production input derivation, preparation safety planning, candidate generation, focus/hysteresis, live wiring and player-facing explanations remain future work. Synthetic domain tests prove selection changes when observed inventory changes; their setup-cost derivation exists only in test code.
