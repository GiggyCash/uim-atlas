...
`data/MethodDefinitionLoader` validates versioned JSON catalogs into immutable `recommendation/MethodDefinition` values through separate synthetic-v1 and production-v3 entry points. Production requires source/review metadata and a caller-supplied canonical item-ID boundary; optional setup stays metadata and cannot affect eligibility. `Requirement` evaluates a numeric fact with its existing `state/Observation` provenance and explicit freshness policy. `MethodEvaluator` separates hard blockers, missing preparation and unknown requirements. `MethodScorer` accepts explicit normalized inputs and returns a weighted breakdown only for available/preparable methods. Required unknown state cannot be offset by a high score.

The recommendation scaffold does not run at startup, select a live recommendation or render UI. Three fixture methods reside exclusively in test resources. Three reviewed Construction methods ship in main resources. Game knowledge, exact item IDs and capability requirements remain in JSON; Java only provides generic observation and evaluation behavior. Artifact tests enforce production/test separation. See the implemented v1/v3 sections of `DATA_SCHEMA.md` for schema and scoring contracts.
...
`workingCapacity` describes a useful ordinary batch and returns independent diagnostics, not gates. Smaller batches may remain possible, and a verified helper profile may establish a different workable payload. Both Mahogany Homes records use a 14-position working assumption; their minimum starting-stock check does not require that batch. Limestone's sourced 20-position collection loop stays real preparation. Generic `inventory.item.<id>.usable_slots` counts ordinary free slots plus directly observed positions occupied by that exact item, preserving inventory provenance. It counts slot entries, never quantities: one large stack contributes one position. Production data must opt into this exact meaning with `capacitySemantics: FREE_PLUS_OBSERVED_EXACT_ITEM_SLOTS`. No item registry, stackability inference, nested-container observation or disposal planning is added.
...
Future live actionability is stricter than rankability: `AVAILABLE` is potentially actionable; `NEEDS_PREP` is potentially actionable only with verified feasible preparation. `NEEDS_PREP` with UNKNOWN preparation feasibility is not yet trustworthy as a live action. `BLOCKED` / `UNKNOWN` are not actionable. No new evaluator status or full PreparationPlanner is introduced.
