...
`ui/PlannerViewModel` translates structured results into bounded display text. Method explanations use the matched goal target, verified setup requirements and structured start location; they do not expose raw catalog reasons, assumptions, start instructions or editorial stop-condition prose. Internal quest readiness remains `READY_TO_HANDOFF`, while the product surface names the quest and labels it `QUEST`; no Quest Helper instruction or handoff implementation term is displayed. `UimAtlasPanel` renders wrapping plain text at its actual parent width, hides its selector for a sole goal, and places alternative title, status and reason on separate lines. Swing never evaluates methods or goals or reads the RuneLite client. Client reads, observation refresh, planning and route posting run on RuneLite's client thread. The immutable view model crosses to Swing's event-dispatch thread. Expiring relevant facts mark their generic observer families dirty before recalculation. Logout, loading, world hop, profile change, account-hash change and plugin shutdown clear the snapshot/result before another account can be displayed. No selected account identifier is stored or logged.
...
Reviewed 2026-09-15: the Plugin Hub marker pinned Shortest Path revision `9953d52745f711a38c9cdd4a00bb1d0d57d1fdea`. Its public incoming `PluginMessage` handler accepts namespace `shortestpath`, message `path`, and `start` / `target` values as RuneLite `WorldPoint` objects. Quest Helper revision `a52646118f0e5ea63a6b3331cefa98087a7b4d6c` independently demonstrates the same protocol. The reviewed Shortest Path source exposes no acknowledgement, availability query or ownership token. Full sources and the route-data review are recorded in [the route-target audit](docs/route-target-audit-v1.md).

`integration/ShortestPathBridge` is the only route transport boundary. A panel click emits a data-only route target to the plugin/controller; the client-thread callback verifies `LOGGED_IN`, reads the local player's current `WorldPoint` at that moment, converts the data coordinate and posts one `path` message. A true bridge result means only that the event was posted, so the UI says “Route requested.” It does not mean Shortest Path is installed, accepted the message, found a path or activated a route.

Atlas never imports Shortest Path classes, reflects into it, reads its configuration, calculates a route, posts automatically, or sends its public `clear` message. Recommendation changes, recalculation, goal changes, logout, sidebar closure and shutdown therefore cannot clear or replace routes created by Quest Helper, Shortest Path itself, another plugin, or the player. The optional integration disappears from the action surface when method data has no verified `routeTarget`; quests never expose an Atlas Route action in v1.
