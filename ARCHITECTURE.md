...
The recommendation scaffold does not run at startup, select a live recommendation or render UI. Three fixture methods reside exclusively in test resources. Three reviewed Construction and six reviewed Herblore methods, plus one reviewed Recipe for Disaster goal graph, ship in main resources. Game knowledge, exact item/quest IDs, recipes and goal prerequisites remain in JSON; Java only provides generic observation and evaluation behavior. Artifact tests enforce production/test separation. See the implemented v1/v3/v4 method and v1 goal sections of `DATA_SCHEMA.md` for schema and scoring contracts.
