...
This is a domain-only scaffold. It does not read the RuneLite client, alter account observation, run at startup, select a live recommendation or render UI. Three fixture methods reside exclusively in test resources. Three reviewed Construction methods now ship in main resources. All game knowledge, exact item IDs and unsupported capability declarations remain in JSON; no new observation subsystem or method-specific Java is added. Artifact tests enforce production/test separation. See the implemented v1/v3 sections of `DATA_SCHEMA.md` for schema and scoring contracts.
