...
The recommendation scaffold does not run at startup, select a live recommendation or render UI. Three fixture methods reside exclusively in test resources. Three reviewed Construction and six reviewed Herblore methods ship in main resources. Game knowledge, exact item IDs, recipes and capability requirements remain in JSON; Java only provides generic observation and evaluation behavior. Artifact tests enforce production/test separation. See the implemented v1/v3/v4 sections of `DATA_SCHEMA.md` for schema and scoring contracts.

`state/AccountStateFacts` projects one normalized snapshot into the generic `recommendation/FactLookup` interface. The dependency direction is `RuneLite Client -> RuneLiteAccountObserver -> AccountState -> AccountStateFacts -> FactLookup -> evaluation/resource flow/scoring/readiness`. The evaluator knows neither account layout nor client APIs. Skill, capability, slot and supported container facts preserve their observations; arbitrary carried item quantities and directly observed exact-item position counts are resolved on request without an item registry or mutable cache.
