...
The domain-only setup derivation below supplies four account-specific factors. With a selected goal, `GoalContext` derives goal progress under the production-goal rule above. Callers still explicitly supply storage/unlock value, risk and optional estimate uncertainty; method efficiency comes from the opt-in verified profile derivation. Missing or invalid values are errors, not zero defaults. Raw XP/hour must not be mixed directly with normalized efficiency. Potential storage value in a definition remains metadata, not proof that this account benefits from an unlock. No live production score is calculated from the synthetic fixtures.
