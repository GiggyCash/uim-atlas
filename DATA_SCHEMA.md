...
Production `start` metadata may additionally contain an optional `routeTarget` object with exactly integer `x`, `y`, and `plane` fields. `x` and `y` must be within the RuneScape world-coordinate boundary `[0,16383]`; `plane` must be in `[0,3]`. Its presence means only that a stable destination was verified for an explicit route request. It does not establish travel availability, proximity, safety, duration, or route acceptance. Synthetic v1 does not accept this field. Route provenance and intentional omissions for the current catalogs are recorded in [the route-target audit](docs/route-target-audit-v1.md).
