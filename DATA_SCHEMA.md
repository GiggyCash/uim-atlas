...
- **Workable batch:** `workingCapacity` describes a useful ordinary batch assumption and returns each predicate's satisfied/missing/unknown diagnostic. Both Mahogany Homes records use 14 ordinary plank positions as an editorial working target, based on the guide's 14/20-position examples. Missing this target does not make smaller batches impossible. A verified helper profile can establish an alternative usable batch even when the ordinary working target is unmet. Capacity describes room, not materials already obtained.
...
Container `<id>` is one lowercase stable segment `[a-z][a-z0-9_]*`; exact item IDs use the existing canonical boundary, including contents-only references. Container observations remain entirely **unsupported** in `AccountStateFacts`, even if the container item is carried. No helper/container observation API is implemented. Future providers must preserve source, time, confidence, account identity/reset boundaries and the relevant capacity units; they cannot infer contents or spare space from ownership. Unknown helper state is not absence; unknown contents are not zero; unknown free capacity is not maximum capacity. Invalid numeric counts also evaluate UNKNOWN.
...
#### Future live actionability boundary

- `AVAILABLE` -> potentially actionable.
- `NEEDS_PREP` + verified feasible preparation -> potentially actionable.
- `NEEDS_PREP` + preparation feasibility UNKNOWN -> not yet trustworthy as a live action.
- `BLOCKED` / `UNKNOWN` -> not actionable.

Ranking eligibility remains separate: `NEEDS_PREP` may still compete in the existing domain ranker when explicit scoring inputs exist. No new evaluator status, PreparationPlanner, live selector, observation integration or UI is introduced here.
...
The domain-only setup derivation below now supplies four account-specific factors. Callers explicitly supply goal progress, storage/unlock value, risk and optional estimate uncertainty; method efficiency is explicitly supplied or comes from the opt-in verified profile derivation above; missing or invalid values are errors, not zero defaults. Raw XP/hour must not be mixed directly with normalized efficiency. Potential storage value in a definition remains metadata, not proof that this account benefits from an unlock. No live production score is calculated from the synthetic fixtures.
